...
async function _addPlayer(roomId, deviceId, name, flag, lane) {
  await redis.hset(`crun:${roomId}:players`, {
    [deviceId]: JSON.stringify({ name, flag: flag ?? null, lane, joinedAt: Date.now() }),
  });
...
async function joinRoom(roomId, deviceId, name, flag) {
  ...
  if (playersRaw && playersRaw[deviceId]) {
    // Already in — update flag if provided
    if (flag) {
  ...
  const shouldStart = currentCount + 1 >= MAX_PLAYERS;
  return { ok: true, shouldStart };
}
...
module.exports = {
  createRoom, joinRoom, setFlag, forceStart,
