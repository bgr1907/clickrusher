...
async function createRace({ creatorDevice, creatorName, mode, winType, winValue, maxPlayers, teams, creatorFlag }) {
  const id  = genId();
  const now = Date.now();
  ...
  await redis.zadd('races:open', now, id);
  return { ok: true, id };
}
...
async function _saveHistory(deviceId, data) {
  await redis.lpush(`user:race:history:${deviceId}`, JSON.stringify(data));
  await redis.ltrim(`user:race:history:${deviceId}`, 0, 49);
  await redis.expire(`user:race:history:${deviceId}`, 365 * 24 * 3600);
}
...
async function getUserRaceHistory(deviceId) {
  const raw = await redis.lrange(`user:race:history:${deviceId}`, 0, 49);
  return (raw || []).map(r => JSON.parse(r));
}

module.exports = {
  createRace, joinRace, beginCountdown, activateRace,
